...
local function _CreateColumns(self)
	...
	for _,id in pairs(self.cfg.selected) do
		log:Debug("handle selected id: %s", id)
		local col = {}
...
local function _SelectedItem(self, row)
	...
	log:Debug("shiftDown [%s] ctrlDown [%s]", shiftDown, ctrlDown)
	local offset = FauxScrollFrame_GetOffset(self.scrollFrame)
...
local function _SetColSelection(info, value)
	if (value) then
		for _,id in pairs(info.arg.cfg.selected) do
			if (id == info.passValue) then
				return;
			end
		end
