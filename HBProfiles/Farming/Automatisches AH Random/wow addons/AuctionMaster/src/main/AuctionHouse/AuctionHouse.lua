...
local function _StopWait(self)
	vendor.Vendor:Debug("stop waiting")
...
local CMDS = {
	scan = {
		...
		disabledFunc = function() return vendor.Scanner:IsScanning() end,
		order = 1
	},
	snipe = {
		type = "execute",
		text = L["Snipe"],
		func = function() vendor.Sniper:OpenSnipeDialog() end,
		order = 3
	},
	conf = {
		type = "execute",
...
local function _CreateOptions()
	vendor.Vendor.options2.args.auctionHouse = {
		type = "group",
		name = L["AuctionHouse"],
		desc = L["AuctionHouse"],
		args = {
			startTab = {
				...
				get = _GetValue,
				set = _SetValue,
				values = {L["Browse"], L["Bids"], L["Auctions"], L["AuctionMaster"], L["Scanner"]},
				arg = "startTab",
			},
		}
	}
	vendor.Vendor.AceConfigRegistry:RegisterOptionsTable("AuctionMaster AuctionHouse", vendor.Vendor.options2.args.auctionHouse)
	vendor.Vendor.AceConfigDialog:AddToBlizOptions("AuctionMaster AuctionHouse", L["AuctionHouse"], "AuctionMaster")
end
...
local function _OnClick(self, button)
	vendor.Vendor:Debug("_OnClick button: %s", button)
    ...
    elseif (button == "RightButton") then
   		self:SelectTab(vendor.Scanner.scanFrame:GetTabId())
    end
end
...
local function _OnUpdate(self, diff)
	...
	if (doBlink) then
		...
		if (self.blinkTime) then
			vendor.Vendor:Debug("_OnUpdate set normal texture")
...
local function _CreateAuctionMasterLogo(self)
	vendor.Vendor:Debug("_CreateAuctionMasterLogo enter")
	...
	if (vendor.Scanner.scanFrame) then
	...
	vendor.Vendor:Debug("_CreateAuctionMasterLogo exit")
end
...
function vendor.AuctionHouse:OnEnable()
	vendor.Vendor:Debug("AuctionHouse:OnEnable enter")
	self.frames = {};
	self.waiters = {}
	self.waitId = 0
...
--	vendor.AuctionHouse.EnsureAuctionHouseUI()
	...
	vendor.Vendor:Debug("AuctionHouse:OnEnable exit")
end
...
function vendor.AuctionHouse:AUCTION_HOUSE_SHOW()
	...
	if (not self.db.profile.lastSeenRelease or self.db.profile.lastSeenRelease ~= vendor.Vendor.version) then
		vendor.Help:Show(vendor.Help.RELEASES_3X)
		--UIErrorsFrame:AddMessage(L["New AuctionMaster release available"], nil, nil, nil, nil, UIERRORS_HOLD_TIME)
		self.db.profile.lastSeenRelease = vendor.Vendor.version
	end
end
...
function vendor.AuctionHouse:CreateTabFrame(name, title, tabTitle, tabFrame)
	vendor.Vendor:Debug("CreateTabFrame");
	...
	vendor.Vendor:Debug("CreateTabFrame numTabs: "..AuctionFrame.numTabs)
	local but = CreateFrame("Button", "AuctionFrameTab"..id, AuctionFrame, "AuctionTabTemplate");
	vendor.Vendor:Debug("CreateTabFrame created tab button with id: "..id)
	but:SetID(id);
...
function vendor.AuctionHouse:IsAuctionHouseTabShown(tabType)
	vendor.Vendor:Debug("IsAuctionHouseTabShown tabType: "..tabType)
	if (AuctionFrame and AuctionFrame:IsVisible()) then
		vendor.Vendor:Debug("type: "..self.type)
		return self.type == tabType
	end
	vendor.Vendor:Debug("return false")
	return false
end
...
function vendor.AuctionHouse:UI_ERROR_MESSAGE(event, message)
	vendor.Vendor:Debug("received emessage: %s", message)
