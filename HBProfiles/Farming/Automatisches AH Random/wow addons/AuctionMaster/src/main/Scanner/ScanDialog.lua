...
local function _CreateBuyButtons(self, frame)
	...
	buyBt = CreateFrame("Button", nil, butGrp, "UIPanelButtonTemplate");
	buyBt.ctrl = self;
	...
	pauseBt = vendor.GuiTools.CreateButton(butGrp, "UIPanelButtonTemplate", BUTTON_WIDTH, 22, L["Pause"], L["Pauses any snipes for the current scan."])
	...
	snipeBt.ctrl = self;
	snipeBt:SetScript("OnClick", function(bt) vendor.Sniper:OpenSnipeDialog(bt.ctrl.name) end);				
	
	contBt = CreateFrame("Button", nil, butGrp, "UIPanelButtonTemplate");
	contBt:SetText(L["Continue"]);
	contBt:SetWidth(SMALL_BUTTON_WIDTH);
	contBt:SetHeight(22);
	contBt:SetPoint("LEFT", snipeBt, "RIGHT", BUTTON_GAP, 0);
	contBt.ctrl = self;
...
local function _CreateAckBuyButtons(self, frame)
	...
	local but1 = CreateFrame("Button", nil, butGrp, "UIPanelButtonTemplate");
	but1:SetText(L["Ok"]);
	but1:SetWidth(BUTTON_WIDTH);
	but1:SetHeight(22);
	but1:SetPoint("BOTTOM", -((BUTTON_WIDTH / 2) + (BUTTON_GAP / 2)), 10);
	but1.ctrl = self;
	but1:SetScript("OnClick", function(bt) 
		local self = bt.ctrl
		if (self.doBid) then
			log:Debug("doBid")
			vendor.Sniper:SnipeForItem("list", self.index, self.bid, 0, self.name, self.count)
	...
	but2 = CreateFrame("Button", nil, butGrp, "UIPanelButtonTemplate");
	but2.ctrl = self;
	but2:SetText(L["Cancel"]);
	but2:SetWidth(BUTTON_WIDTH);
	but2:SetHeight(22);
	but2:SetPoint("LEFT", but1, "RIGHT", BUTTON_GAP, 0);
	but2:SetScript("OnClick", function(bt)
		vendor.Sniper:StopPendingBuy();
		bt.ctrl.decisionMade = true;  
	end);
