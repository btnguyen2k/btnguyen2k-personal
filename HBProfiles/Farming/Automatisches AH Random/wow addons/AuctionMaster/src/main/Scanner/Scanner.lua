...
--					order = 3,
--			},
--			snipeAverageMinCount = {
--				type = "range",
--					name = L["Snipe average auctions count"],
--					desc = L["How often has the item in question to be seen in auction house, until it may be sniped according to the market price."],
--					get = _GetValue,
--					set = _SetValue,
--					min = 0,
--					max = 10000,
--					step = 10,
--					arg = "snipeAverageMinCount",
--					order = 3.1,
--			},
			minimumProfit = {
				type = "range",
...
function vendor.Scanner:OnInitialize()
	...
	self.db = vendor.Vendor.db:RegisterNamespace("Scanner2", {
		...
		profile = {
    		...
    		getAll = true,
    		scanSpeed = vendor.ScanFrame.SCAN_SPEED_HURRY
		}
	});
	_MigrateDb(self)
	_CreateOptions();	
...
--]]
function vendor.Scanner:Scan(itemLink, silent, func, arg, modules, waitForOwners)
	log:Debug("Scan itemLink [%s] waitForOwners [%s]", itemLink, waitForOwners)
	if (self.scanTask) then
		...
		local queryInfo = {itemLinkKey = itemLinkKey, name = name, minLevel = "", maxLevel = "", 
			invTypeIndex = 0, classIndex = 0, subclassIndex = 0, isUsable = 0, qualityIndex = 0}
...
function vendor.Scanner:PlaceAuctionBid(ahType, auctions, possibleGap)
	...
	for i=#auctions,1,-1 do
		...
		if (info.index and info.index > 0) then
			...
			local bid, bought, name, _, count, minBid, minIncrement, buyout, bidAmount
			for idx=info.index,firstIdx,-1 do
				name, _, count, _, _, _, minBid, minIncrement, buyout, bidAmount = GetAuctionItemInfo(ahType, idx)
				bid = minBid
