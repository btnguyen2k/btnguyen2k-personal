...
local RELEASE_NOTES = HTML_PREFIX..L["3xReleases"]..HTML_SUFFIX
...
vendor.Help.RELEASES_3X = "3xReleases"
...
local function _OnGroupSelected(widget, event, value)
	local self = widget.userdata.obj
 	if (value == self.RELEASES_3X) then
 		self.html:SetText(RELEASE_NOTES)
 		self.frame:SetStatusText(L["3xReleasesStatus"])
 	elseif (value == self.DOCUMENTATION) then
...
local function _CreateFrame(self)
	...
	local tree = { 
    	...
    	{value = "3xReleases", text = L["3.x Releases"]},
    }
