...
--			log:Debug("_MatchesScan exit because of name (scan: %s auctionb: %s", scan.name or "", name or "")
		...
		if (scan.invType or scan.class or scan.subclass or scan.isUsable or scan.qualityIndex > 0) then
			if (doLog) then
			...
			if (not itemName) then
				...
				if (scan.isUsable == 1 and not self.canUseEval:IsUsable(itemLink)) then
					if (doLog) then
						log:Debug("_MatchesScan exit because of usable scan.isUsable: %s isUsable: %s", scan.isUsable, isUsable)
					end
					break
				end
			end
...
--[[
	Updates the snapshot with all own auctions.
--]]
local function _UpdateOwnAuctions(self)
	log:Debug("_UpdateOwnAuctions")
	local snapshot = _GetSnapshot(self)
	local owner = UnitName("player")
	local neutralAh = vendor.AuctionHouse:IsNeutral()
	_CleanupSnapshot(self, snapshot, neutralAh)
	coroutine.yield()
	local ownAuctions = wipe(self.tmpList1)
	_CopyOwnAuctions(self, ownAuctions)
	coroutine.yield()
	local snapshotOwnAuctions = wipe(self.tmpList2)
	_SearchOwnAuctions(self, snapshotOwnAuctions, snapshot, owner)
	coroutine.yield()
	-- determine the auctions in the snapshot, that has to be archived
	local tmp = wipe(self.tmpList3) 
	local archived = wipe(self.tmpList4)
	vendor.AddToTable(tmp, snapshotOwnAuctions)
	_SubtractAuctions(tmp, ownAuctions)
	log:Debug("remove [%s] auctions", #tmp)
	if (#tmp > 0) then
    	-- sort according to index, for being able to delete them in snapshot
    	table.sort(tmp, function(a, b) return a.index > b.index end)
    	for i=1,#tmp do
    		table.insert(archived, snapshot[tmp[i].index])
    		table.remove(snapshot, tmp[i].index)
    	end
    	_ArchiveAuctions(self, archived, neutralAh)
    end
	
	-- determine the own auctions, that has to be added
	local added = wipe(self.tmpList3) 
	_SubtractAuctions(ownAuctions, snapshotOwnAuctions)
	log:Debug("add [%s] auctions", #ownAuctions)
	if (#ownAuctions > 0) then
    	for i=1,#ownAuctions do
    		local data = _PackOwnAuctionInfo(ownAuctions[i], owner)
    		table.insert(added, data)
    	end
    	_ReinsertAuctions(self, snapshot, added)
    	table.sort(snapshot)	
    end
    coroutine.yield()
   	_NotifyUpdate(self)
   	log:Debug("_UpdateOwnAuctions exit")
end

--[[
	Triggers an own auction update, if needed.
--]]
local function _TriggerUpdateOwnAuctions(self)
	local update = self.ownAuctionUpdate
	if (update > (self.ownAuctionDone + 0.0001)) then
		log:Debug("ownAuctionUpdate [%s] ownAuctionDone [%s]", self.ownAuctionUpdate, self.ownAuctionDone)
		
		-- this seem to help with missing update-events 	
--    	local _, tab = vendor.AuctionHouse:GetCurrentAuctionHouseTab()
--    	if (tab ~= 3) then
--    		vendor.AuctionHouse:SelectTab(3)
--    		vendor.AuctionHouse:SelectTab(tab)
--    	end
...
--		end
		...
		self.ownAuctionDone = self.ownAuctionUpdate
		vendor.TaskQueue:Execute(_UpdateOwnAuctions, self)
	end
end
	...
	if (not info) then
    	...
    	if (idx) then
    		...
    		for i=idx,#snapshot do
    			local key, count, minBid, buyout, name, _, _, _, bidAmount, _, _, _, time = _Unpack(snapshot[i]) 
...
--    		log:Debug("adjustPrices [%s] buyouts [%s] values [%s]", adjustPrices, #buyouts, values)
	...
	return info.avgBid, info.avgBuyout, info.lowerBid, info.lowerBuyout, info.upperBid, 
    	info.upperBuyout, info.numAuctions, info.numBuyouts, info.leastCurrentTime
end
...
function vendor.Gatherer:StopScan(itemLinkKey, complete)
	log:Debug("StopScan itemLinkKey [%s] complete [%s]", itemLinkKey, complete)
...
--[[
	Own auction list has been updated. We will update the snapshot accordingly.
--]]
function vendor.Gatherer:OWN_AUCTIONS_UPDATE()
	log:Debug("OWN_AUCTIONS_UPDATE")
	self.ownAuctionUpdate = GetTime() + 1
end

--[[
	Retrieves a ScanSet for the given item.
--]]
function vendor.Gatherer:GetCurrentAuctions(itemLinkKey, neutralAh)
	local index = {}
	local snapshot = self.db.factionrealm.snapshot
    if (neutralAh) then
    	snapshot = self.db.realm.snapshot
    ...
    if (idx) then
		...
		for i=idx,#snapshot do
			...
			local data = vendor.ScanResults.Pack(key, time or GetTime(), timeLeft, count or 1, minBid, minIncrement or 0, buyout or 0, bidAmount or 0, owner or "", highBidder or "", 0)
			table.insert(index, data) 
		end
	end
	return vendor.ScanSet:new(index, neutralAh, itemLinkKey)
end

--[[
	Returns the below described information for current auctions of the specified item.
    avgBid, avgBuyout, lowerBid, lowerBuyout, upperBid, upperBuyout, numAuctions, numBuyouts, leastCurrentTime
--]]
