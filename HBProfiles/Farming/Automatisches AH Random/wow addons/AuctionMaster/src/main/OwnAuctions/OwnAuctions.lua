...
local function _CancelAuctionsTable(self, auctions)
	...
	for k,v in pairs(auctions) do
		local origName, _, origCount, _, _, _, origMinBid, _, origBuyout, origBidAmount = GetAuctionItemInfo("owner", v.index)
...
local function _CancelUndercut(self)
	...
	for row=1,self.itemModel:Size() do
		local index, _, _, name, _, count, bid, buyout, lowerBuyout, saleStatus, outDated = self.itemModel:Get(row)
...
local function _OnUpdate(frame)
	...
	if (self.doUpdate) then
		if (self.delay) then
			if (GetTime() - self.delay < 5) then
				log:Debug("delay update")
				return
			end
		end
...
--]]
function vendor.OwnAuctions:CHAT_MSG_SYSTEM(event, message)
--	log:Debug("CHAT_MSG_SYSTEM [%s]", message)
...
--[[
	Notifies any listener about an update through events.
--]]
function vendor.OwnAuctions:NotifyUpdate()
	self:SendMessage("OWN_AUCTIONS_UPDATE")
end
...
function vendor.OwnAuctions:CancelAuctions(auctions)
	...
	log:Debug("2")
		...
		for j=1,self.itemModel:Size() do
			if (not tmp[j]) then
    			local index, itemLinkKey, _, name, _, count, bid, buyout, lowerBuyout, saleStatus, _, 
    					minBid = self.itemModel:Get(j)
    			...
    			if (itemLinkKey == info.itemLinkKey and minBid == info.minBid and
    				...
    				if (saleStatus == 1) then
    					...
    					table.insert(cancelAuctions, {index = index, name = name, count = count})
