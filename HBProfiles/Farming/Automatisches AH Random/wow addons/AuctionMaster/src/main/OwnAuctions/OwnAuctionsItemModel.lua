...
local function _Update(self)
	...
	for i=1,GetNumAuctionItems("owner") do
		local name, texture, count, quality, _, level, minBid, minIncrement, buyoutPrice, bidAmount, highBidder, _, saleStatus = GetAuctionItemInfo("owner", i)
    ...
    vendor.OwnAuctions:NotifyUpdate()
...
function vendor.OwnAuctionsItemModel.prototype:GetValues(row, id)
	...
	elseif (id == vendor.OwnAuctionsItemModel.CURRENT_AUCTIONS) then
		if (outdated and info.currentAuctions) then
			rtn1 = GRAY_FONT_COLOR_CODE..info.currentAuctions..FONT_COLOR_CODE_CLOSE
		else
