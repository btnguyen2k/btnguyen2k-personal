...
local GOLD_ICON_CODE = "|TInterface\\MoneyFrame\\UI-GoldIcon:14:14:2|t"
local SILVER_ICON_CODE = "|TInterface\\MoneyFrame\\UI-SilverIcon:14:14:2|t"
local COPPER_ICON_CODE = "|TInterface\\MoneyFrame\\UI-CopperIcon:14:14:2|t"
...
function vendor.Format.FormatMoney(copper, short)
	copper = math.floor(math.max(0, copper or 0));
   	...
   	local copper = math.fmod(copper, 100);
   	local rtn = ""
...
--    		rtn = rtn..WHITE_COLOR_CODE..copper..COPPER_ICON_CODE.." "
--    	end 
--    else
       	if (gold > 0) then
       	...
       	if (copper > 0 and (not short or gold < 1)) then
          	rtn = rtn..COPPER_COLOR_CODE..copper.."c";
       	end
--	end
...
function vendor.Format.FormatMoneyGrayedOut(copper, short)
	copper = math.floor(math.max(0, copper or 0));
   	...
   	local rtn = ""
   	if (gold > 0) then
		rtn = rtn..GRAY_FONT_COLOR_CODE..gold.."g";
   	end
   	if (silver > 0 and (not short or gold < 1000)) then
      	rtn = rtn..GRAY_FONT_COLOR_CODE..silver.."s";
   	end
   	if (copper > 0 and (not short or gold < 1)) then
      	rtn = rtn..GRAY_FONT_COLOR_CODE..copper.."c";
   	end
   	if (string.len(rtn) == 0) then
		rtn = GRAY_FONT_COLOR_CODE.."--"
   	end
   	return rtn..FONT_COLOR_CODE_CLOSE
end
...
function vendor.Format.FormatMoneyValues(copper1, copper2, short)
	return vendor.Format.FormatMoney(copper1, short).." ("..vendor.Format.FormatMoney(copper2, short)..")"
end
...
function vendor.Format.FormatMoneyValuesGrayedOut(copper1, copper2, short)
	return vendor.Format.FormatMoneyGrayedOut(copper1, short).." ("..vendor.Format.FormatMoneyGrayedOut(copper2, short)..")"
end
