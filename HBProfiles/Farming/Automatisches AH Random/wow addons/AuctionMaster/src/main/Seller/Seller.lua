...
local function _RefreshClicked(self)
	...
	if (self.item) then
		...
		vendor.Scanner:Scan(self.item.link, true, nil, nil, nil, waitForOwners)
	elseif (self.lastItem) then
		...
		vendor.Scanner:Scan(self.lastItem.link, true, nil, nil, nil, waitForOwners)
	end
	self.itemTable:Show()
...
local function _CancelAuctions(self)
	...
	for _,row in pairs(self.itemModel:GetSelectedItems()) do
		local itemLinkKey, _, _, count, minBid, _, buyout = self.itemModel:Get(row)
		table.insert(auctions, {itemLinkKey = itemLinkKey, count = count, minBid = minBid, buyout = buyout})
	end
...
local function _Bid(self)
	local auctions = {}
	for _,row in pairs(self.itemModel:GetSelectedItems()) do
		local itemLinkKey, _, _, count, minBid, minIncrement, buyout, bidAmount = self.itemModel:Get(row)
   		local bid = minBid
		...
		local name = GetItemInfo(itemLink)
   		local info = {itemLink = itemLink, name = name, count = count, minBid = minBid, buyout = buyout, bid = bid, reason = L["Bid"]}
...
local function _Buyout(self)
	...
	for _,row in pairs(self.itemModel:GetSelectedItems()) do
		...
		local itemLinkKey, _, _, count, minBid, _, buyout = self.itemModel:Get(row)
   		local itemLink = vendor.Items:GetItemLink(itemLinkKey)
		local name = GetItemInfo(itemLink)
   		local info = {itemLink = itemLink, name = name, count = count, minBid = minBid, buyout = buyout, bid = buyout, reason = L["Buy"]}
   		table.insert(auctions, info)
	end
	vendor.Scanner:BuyScan(auctions, _RefreshClicked, self)
end
...
local function _GetUndercutPrice(self, itemLink)
	local _, _, minBid, buyout = vendor.Statistic:GetCurrentAuctionInfo(itemLink, vendor.AuctionHouse:IsNeutral())
	return _ModifyPrices(minBid, buyout)
end
...
local function _CreateAuctionButtons(self)
	...
	self.durationDropDown:SetPoint("TOPLEFT", 10, -180);
	...
	self.stackDropDown:SetPoint("TOPLEFT", 118, -180);
	...
	self.prizingDropDown:SetPoint("TOPLEFT", SalesFrameDuration, "BOTTOMLEFT", 0, -13);
