...
local function _SearchItem(auctionType, index, bid, buyout, name, count)
	local itemName, itemCount, minBid, minIncrement, buyoutPrice, bidAmount, _
	for i=index,1,-1 do
		itemName, _, itemCount, _, _, _, minBid, minIncrement, buyoutPrice, 
...
function vendor.Sniper:ItemScanned(index, itemLink, itemName)
	if (not self.paused) then
		...
		if (auctionName == itemName and playerName ~= owner) then
			local bid = minBid;
			if (bidAmount > 0) then
			...
			if (_MayBuy(itemName, count, bid, buyout)) then
				if (self.exclusiveSniper) then
					...
					for i=1,#self.snipers do
						...
						rtn, reason = sniper:Snipe(itemLink, itemName, count, bid, buyout, highBidder)
						if (rtn) then
			...
			return rtn, reason, sniperId, doBid, doBuyout
		end
	end
	return nil
end
...
--[[
	Auto buys/bids the given list of auctions. The items in the table must have the following fields:
	itemLinkKey, count and minBid or buyout depending on whether they should be bidded or bought.
--]]
function vendor.Sniper:BidOrBuyAuctions(auctions)
	local task = vendor.BidOrBuyTask:new(auctions)
