 ...
 */
/**
 * Modified by Thanh Ba Nguyen:
 * - Added support to title block
 * - Fix a IE bug with using <code> instead of <pre> or text area
 */
//
	...
	highlight : function(globalParams, element)
	{
		...
		for (var i = 0; i < elements.length; i++) 
		{
			...
			if (navigator.appName.indexOf('MSIE') != -1 || navigator.appName.indexOf('Microsoft Internet Explorer') != -1) {
				propertyName = 'innerText'; //for Internet Explorer: use innerText instead of innerHTML
			}
			highlighter.highlight(target[propertyName], params);
			
			var result = highlighter.div;
			
			//TXBB: add title block
			if ( params['title'] ) {
